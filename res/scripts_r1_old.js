...
Site.Home = 
{
	...
	init: function()
	{
		...
		$('sidebar').addEvents(
		{
			'mouseover:relay(ul#socialfeed > li)': function(e, el)
			{
				el.getElement('ul.meta').tween('height', null, 20);
			},
			'mouseout:relay(ul#socialfeed > li)': function(e, el)
			{
	...
	Twitter: 
	{
		...
		count: 6,
		// URL to retrieve tweets from
		url: 'http://api.twitter.com/1/statuses/user_timeline/{username}.json',
		// URL to a user's status
		user_url: 'http://twitter.com/{username}/status/{id}',
		...
		{
			new Request.JSONP({
				url: this.url.substitute({username: this.username}),
				data: {
					'count': this.count,
					'include_rts': 1
				},
		...
		{			
			...
			data.each(function(item)
			{
				...
				// Username link
				new Element('a', {'href': this.user_url.substitute({username: this.username, id: item.id}), 'html': this.username + ': '}).inject(tweet, 'top');
				// Date
...
{
	/**
	 * Turn any http:// links into clickable links
	 */
	linkify: function()
	{
	...
	/**
	 * Turn "@username" and "#hashtag" into links
	 */
...
var SocialFeed = new Class(
{
	...
	processResponse: function(response)
	{
		...
		response.each(function(item)
		{
			...
			// The sub-data
			var meta = new Element('ul', {'class': 'meta'});
			...
			if (item.url)
				new Element('li', { html: '<a href="' + item.url + '">View</a>' }).inject(meta);
